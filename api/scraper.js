...
async function scraper( width1, length1, height1, loadheight1) {
  try {
    ...
    await page.goto('https://www.onpallet.com/', { waitUntil: 'load', timeout: 0 });
    ...
    await page.$eval('#p_width', (el, width1) => { el.value = width1 }, width1);
    await page.waitFor(2000);

    const selector2 = '#p_length';
    await page.waitForSelector(selector2);
    // await page.type('#p_width', '');
    await page.$eval('#p_length', (el, length1) => { el.value = length1 }, length1);
    await page.waitFor(2000);
    const selector3 = '#p_height';
    await page.waitForSelector(selector3);
    // await page.type('#p_width', '');
    await page.$eval('#p_height', (el, height1) => { el.value = height1 }, height1);
    ...
    await page.$eval('#p_loadheight', (el, loadheight1) => { el.value = loadheight1 }, loadheight1);
    ...
    const grupos = await page.evaluate(() => Array.from(document.querySelectorAll('td'), element => element.textContent));
    //console.log(grupos)
    dic = {}
    ...
    await cloudinary.uploader.upload('scren'+n+'.png', function (error, result) { b1 = result });
